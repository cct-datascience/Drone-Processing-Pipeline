...
def get_fields():
    ...
    """
    return ('local_datetime', SENSOR_NAME, 'access_level', 'species', 'site',
            'citation_author', 'citation_year', 'citation_title', 'method_name')
...
def all_plot_names_found(plot_name, column_names):
...
class PlotExtractor(TerrarefExtractor):
    """Extractor for clipping georeferenced images to plot boundaries via a shape file and
        calling an algorithm on the clipped portions.
        ...
        try:
            ...
            if self.experiment_metadata:
                if 'extractors' in self.experiment_metadata:
                    ...
                    if SENSOR_NAME and SENSOR_NAME in extractor_json:
            ...
            if dbffile:
                ...
                if not plot_name_idx is None:
                    if not all_plot_names_found(plot_name_idx, column_names):
            ...
            out_csv = os.path.join(os.path.dirname(rootdir),
                                   base_name.replace(".tif", "_" + SENSOR_NAME + "_shapefile.csv"))
            out_geo = os.path.join(os.path.dirname(rootdir),
                                   base_name.replace(".tif", "_" + SENSOR_NAME + "_geo.csv"))
            ...
            while feature:
                ...
                for filename in imagefiles:
                    ...
                    try:
                        ...
                        calc_value = extractor.calculate(rollaxis(clip_pix, 0, 3))
                        ...
                        traits[SENSOR_NAME] = str(calc_value)
